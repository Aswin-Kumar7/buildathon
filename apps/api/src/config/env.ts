...
const envSchema = z.object({
  ...
  /**
   * Lets the replay endpoints run under NODE_ENV=production.
   *
   * Off by default and meant to stay off for a merchant: synthetic traffic in the same database
   * as real traffic makes the deployment's own figures unquotable. The hosted demo sets it,
   * because it exists to be driven by strangers and nothing cites its database — the published
   * numbers are generated by CI from the corpus, in a separate process, and never read from here.
   */
  ALLOW_REPLAY_IN_PRODUCTION: envBoolean('false'),
